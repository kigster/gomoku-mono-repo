...
`.features/003.multiplayer-architecture-and-data-model.done/plan.md` §9.
These tests are intentionally red right now because Stage 3 has not yet
implemented the routes/migration. Once Stage 3 makes them green, do not
modify them.
...
# (see .features/009.choose-game-type-modal-and-invite-link.done/plan.md)
# ---------------------------------------------------------------------------
...
async def test_move_oob_uses_400_not_422_for_high_coords(
    ...
    Per .features/008.multiplayer-pr-hardening-checklist.done/plan.md
    item #7 — the wire contract is one consistent OOB status, not a mix
    of Pydantic 422 / handler 400.
    """
    game = await _start_in_progress_game(
