...
- `.features/003.multiplayer-architecture-and-data-model.done/plan.md`
  §4 for the original API surface and §5 for the concurrency rules.
- `.features/009.choose-game-type-modal-and-invite-link.done/plan.md`
  for the host-chooses / guest-chooses color flow, the 15-minute invite
  expiry, and the cancel endpoint.
- `.features/008.multiplayer-pr-hardening-checklist.done/plan.md` for
  the issues that this version addresses (collision retry on code
  generation, board-size move validation, game_type discriminator on
  the `games` history table, surfacing of cancellation/expiry to the
  client, etc.).
...
async def _expire_if_stale(conn, code: str) -> None:
    ...
    state change on their next poll. Per
    `.features/009.choose-game-type-modal-and-invite-link.done/plan.md` §3
    — no background sweeper required for the modal flow.
    """
...
async def _write_finished_games_rows(
    ...
    *,
    ...
    sentinels (see migration 0006 +
    `.features/008.multiplayer-pr-hardening-checklist.done/plan.md` #1).
    """
