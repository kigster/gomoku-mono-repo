...
const char *json_number_chars =
    "0123456789.+-eE"; /* Unused, but part of public API, drop for 1.0 */
...
static inline const struct json_object_object *
JC_OBJECT_C(const struct json_object *jso) {
  return (const void *)jso;
}
static inline struct json_object_array *JC_ARRAY(struct json_object *jso) {
  return (void *)jso;
}
static inline const struct json_object_array *
JC_ARRAY_C(const struct json_object *jso) {
  return (const void *)jso;
}
static inline struct json_object_boolean *JC_BOOL(struct json_object *jso) {
  return (void *)jso;
}
static inline const struct json_object_boolean *
JC_BOOL_C(const struct json_object *jso) {
  return (const void *)jso;
}
static inline struct json_object_double *JC_DOUBLE(struct json_object *jso) {
  return (void *)jso;
}
static inline const struct json_object_double *
JC_DOUBLE_C(const struct json_object *jso) {
  return (const void *)jso;
}
static inline struct json_object_int *JC_INT(struct json_object *jso) {
  return (void *)jso;
}
static inline const struct json_object_int *
JC_INT_C(const struct json_object *jso) {
  return (const void *)jso;
}
static inline struct json_object_string *JC_STRING(struct json_object *jso) {
  return (void *)jso;
}
static inline const struct json_object_string *
JC_STRING_C(const struct json_object *jso) {
  return (const void *)jso;
}
...
json_object_new(enum json_type o_type, size_t alloc_size,
...
static int json_escape_str(struct printbuf *pb, const char *str, size_t len,
                           int flags) {
...
struct json_object *json_object_new_object(void) {
  ...
  if (!jso)
    return NULL;
  jso->c_object = lh_kchar_table_new(JSON_OBJECT_DEF_HASH_ENTRIES,
...
int json_object_object_add_ex(struct json_object *jso, const char *const key,
  ...
  existing_entry = (opts & JSON_C_OBJECT_ADD_KEY_IS_NEW)
                       ? NULL
                       : lh_table_lookup_entry_w_hash(JC_OBJECT(jso)->c_object,
                                                      (const void *)key, hash);
  ...
  if (!existing_entry) {
    const void *const k = (opts & JSON_C_OBJECT_ADD_CONSTANT_KEY)
                              ? (const void *)key
                              : strdup(key);
    if (k == NULL)
      return -1;
...
size_t json_c_object_sizeof(void) { return sizeof(struct json_object); }
...
static int json_object_double_to_json_string_format(struct json_object *jso,
  ...
  if (isnan(jsodbl->c_double)) {
    ...
    if (size < (int)sizeof(buf) - 2 && looks_numeric &&
...
static int json_object_string_to_json_string(struct json_object *jso,
  ...
  printbuf_strappend(pb, "\"");
  json_escape_str(pb, get_string_component(jso), len < 0 ? -(ssize_t)len : len,
                  flags);
  printbuf_strappend(pb, "\"");
  if (flags & JSON_C_TO_STRING_COLOR)
    printbuf_strappend(pb, ANSI_COLOR_RESET);
...
static struct json_object *_json_object_new_string(const char *s,
                                                   const size_t len) {
  size_t objsize;
  ...
  jso = (struct json_object_string *)json_object_new(
      json_type_string, objsize, &json_object_string_to_json_string);
...
_json_object_get_string_len(const struct json_object_string *jso) {
  ssize_t len;
  len = jso->len;
...
static int _json_object_set_string_len(json_object *jso, const char *s,
                                       size_t len) {
  char *dstbuf;
  ssize_t curlen;
  ssize_t newlen;
...
struct json_object *json_object_array_bsearch(const struct json_object *key,
  ...
  result = (struct json_object **)array_list_bsearch(
      (const void **)(void *)&key, JC_ARRAY_C(jso)->c_array, sort_fn);
...
int json_object_array_insert_idx(struct json_object *jso, size_t idx,
                                 struct json_object *val) {
  assert(json_object_get_type(jso) == json_type_array);
...
int json_object_array_del_idx(struct json_object *jso, size_t idx,
                              size_t count) {
  assert(json_object_get_type(jso) == json_type_array);
...
struct json_object *json_object_array_get_idx(const struct json_object *jso,
                                              size_t idx) {
  assert(json_object_get_type(jso) == json_type_array);
  return (struct json_object *)array_list_get_idx(JC_ARRAY_C(jso)->c_array,
                                                  idx);
}
...
static int json_object_copy_serializer_data(struct json_object *src,
  ...
  // else if ... other supported serializers ...
    ...
    _json_c_set_last_err("json_object_copy_serializer_data: unable to copy "
                         "unknown serializer data: "
                         "%p\n",
...
int json_c_shallow_copy_default(json_object *src, json_object *parent,
  ...
  case json_type_string:
    *dst = json_object_new_string_len(
        get_string_component(src), _json_object_get_string_len(JC_STRING(src)));
    break;
 ...
 */
static int json_object_deep_copy_recursive(
    struct json_object *src, struct json_object *parent,
    const char *key_in_parent, size_t index_in_parent, struct json_object **dst,
...
int json_object_deep_copy(struct json_object *src, struct json_object **dst,
  ...
  rc = json_object_deep_copy_recursive(src, NULL, NULL, UINT_MAX, dst,
                                       shallow_copy);
  if (rc < 0) {
    json_object_put(*dst);
    *dst = NULL;
