...
struct json_object *json_tokener_parse_ex(struct json_tokener *tok,
  ...
  redo_char:
    ...
    case json_tokener_state_inf: /* aka starts with 'i' (or 'I', or "-i", or
                                    "-I") */
    {
    ...
    case json_tokener_state_escape_unicode: {
      ...
      if (tok->high_surrogate) {
        if (IS_LOW_SURROGATE(tok->ucs_char)) {
          ...
          tok->ucs_char =
              DECODE_SURROGATE_PAIR(tok->high_surrogate, tok->ucs_char);
        } else {
...
static json_bool json_tokener_validate_utf8(const char c,
                                            unsigned int *nBytes) {
  unsigned char chr = c;
  if (*nBytes == 0) {
    if (chr >= 0x80) {
...
size_t json_tokener_get_parse_end(struct json_tokener *tok) {
  assert(tok->char_offset >=
         0); /* Drop this line when char_offset becomes a size_t */
