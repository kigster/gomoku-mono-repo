...
1. **Use singular names** - `USER` not `USERS`, `ORDER` not `ORDERS`
1. **Define all constraints** - Document PKs, FKs, UKs, NOT NULL
1. **Show cardinality accurately** - Be precise about one-to-many vs many-to-many
1. **Include timestamps** - created_at, updated_at for auditing
1. **Document computed columns** - Mark calculated/derived values
1. **Add meaningful comments** - Use quotes for constraints and descriptions
1. **Consider junction tables** - Explicitly model many-to-many relationships
1. **Use appropriate types** - Match database-specific types
1. **Show indexes** - Document UK (unique keys) beyond PKs
...
1. **Use surrogate keys** - UUID or auto-increment integers as PKs
1. **Index foreign keys** - Essential for join performance
1. **Plan for soft deletes** - Add deleted_at columns instead of hard deletes
1. **Version critical data** - Maintain history for important entities
1. **Set appropriate defaults** - created_at, status, boolean flags
1. **Consider denormalization** - Counts and cached values for performance
1. **Use enum/check constraints** - Enforce valid values at database level
