 ...
 * `.features/008.multiplayer-pr-hardening-checklist.done/plan.md` item #6 — every join failure must surface as
...
function buildBoard(size: number, moves: [number, number][]): CellValue[][] {
...
export default function MultiplayerGamePage({
  ...
  useEffect(() => {
    ...
    const isTerminal = ["finished", "cancelled", "abandoned"].includes(
      game.state,
    );
    const wasInProgress =
      prev !== null && !["finished", "cancelled", "abandoned"].includes(prev);
    ...
    if (game.state === "abandoned" && prev === "in_progress" && !opponentLeft) {
  ...
  const interactive =
    isParticipantView(game) && game.state === "in_progress" && game.your_turn;
...
function GameOverPanel({
  ...
  const opponentUsername =
    game.host.username === username
      ...
      : game.host.username;
  const youWon = game.your_color !== null && game.winner === game.your_color;
...
function PlayerHeader({ game }: { game: MultiplayerGameView }) {
  ...
  return (
    <div className="flex flex-wrap gap-4 justify-center text-neutral-300 text-sm">
      ...
      <div>
        Guest:{" "}
        <span className="text-amber-300 font-semibold">{guestLabel}</span>
      </div>
      {yourSide && (
        <div>
          You play:{" "}
          <span className="text-amber-300 font-semibold">{yourSide}</span>
        </div>
      )}
    </div>
  );
