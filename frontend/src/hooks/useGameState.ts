...
export function useGameState(settings: GameSettings) {
  ...
  const startGame = useCallback(
    async (gameId?: string) => {
      ...
      const timeoutSec =
        settings.aiTimeout !== "none" ? parseInt(settings.aiTimeout) : 0;
  ...
  const makeMove = useCallback(
    async (row: number, col: number) => {
      ...
      const timeoutSec =
        settings.aiTimeout !== "none" ? parseInt(settings.aiTimeout) : 0;
      const timeoutMs = timeoutSec > 0 ? (timeoutSec + 5) * 1000 : undefined;
      const aiMoveSucceeded = await sendToServer(
        newState,
        timeoutMs,
        timeoutSec,
      );
  ...
  const undoMove = useCallback(() => {
    ...
    const movesToRemove =
      gameState.moves.length >= 2 ? 2 : gameState.moves.length;
