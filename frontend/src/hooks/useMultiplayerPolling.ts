...
// expired" to the user. See .features/008.multiplayer-pr-hardening-checklist.done/plan.md item #5. The poll
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (!cancelled && !stoppedRef.current) {
        setTimeout(
          tick,
          pollingIntervalForElapsedMs(Date.now() - startedAtRef.current),
        );
      }
    };
    tick();
